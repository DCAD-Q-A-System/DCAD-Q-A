  ...
  useEffect(() => {
    AXIOS_INSTANCE.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        if (!error.response) {
          setToast("Network error", "network error, check connection", VARIANT.DANGER, true);
        } else {
          switch (error.response.status) {
            case 400:
              setToast("General error", "check your details", VARIANT.DANGER, true);
              break;
            case 500:
              setToast("Server error", "Server error, contact admin", VARIANT.DANGER, true);
              break;
            case 409:
              setToast("General error", "Credentials already used", VARIANT.DANGER, true);
              break;
          }
        }
...
export default App;
function dispatch(arg0: { payload: { data: import("./utils/interfaces").LoginResponse | null; }; type: "login/setData"; }) {
